...
namespace YoungMessaging.EventBus
{
    public class PubSubBusConsumer : IBusConsumer, IBusProducer
    {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        await subscriber.StartAsync(async (PubsubMessage message, CancellationToken token) =>
                        {
                            ...
                            EventResult result = EventResult.Failure;
                            try
                            {
                                result = await handler.Handle(eventMessage, token);
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine(ex.Message);
                ...
                while (true)
                {
                    try
                    {
                        ...
                        await subscriber.StartAsync(async (PubsubMessage message, CancellationToken token) =>
                        {
                            ...
                            EventResult result = await handler.Handle(events, token);
                            return result == EventResult.Success ? SubscriberClient.Reply.Ack : SubscriberClient.Reply.Nack;
                        });
                    }
                    catch (Exception ex)
        ...
        private void CreateSubscription(string topic, string subscription)
        {
            ...
            try
            {
                Task<Subscription> actualSub = subscriberService.GetSubscriptionAsync(subscriptionName);
                if (actualSub != null)
                {
                    return;
                }

                _ = subscriberService.CreateSubscription(subscriptionName, topicName, pushConfig: null, ackDeadlineSeconds: 20);
            }
